"""
分组统计工具 – 支持从原始数据生成统计报表，或从已生成的 Excel 重建 HTML。
支持命令行参数：
  python main.py               # 默认模式：查找数据文件，生成 Excel 和 HTML
  python main.py --from-excel  # 从已有的分组统计结果.xlsx 生成 HTML
"""
...
def find_data_file() -> Optional[str]:
    """
    在 main.py 同级目录下的 data 文件夹中，查找包含“接单明细”的 Excel 文件。
    返回第一个匹配文件的完整路径，若无匹配则返回 None。
    """
    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_dir = os.path.join(script_dir, 'data')
    if not os.path.exists(data_dir):
        return None
    pattern = os.path.join(data_dir, '*接单明细*.xlsx')
    files = glob.glob(pattern)
    # 也支持 .xls 格式？
    pattern_xls = os.path.join(data_dir, '*接单明细*.xls')
...
def rebuild_df_from_excel(excel_path: str, sheet_name: str) -> pd.DataFrame:
    ...
    返回包含 attrs 的 DataFrame。
    """
    # 读取元数据
    meta_df = pd.read_excel(excel_path, sheet_name='元数据', header=None, index_col=0)
    sorted_regions = json.loads(meta_df.loc['sorted_regions', 1])
    region_data = json.loads(meta_df.loc['region_data', 1])

    # 读取主数据（从第3行开始，即跳过前两行表头）
    df_data = pd.read_excel(excel_path, sheet_name=sheet_name, header=None, skiprows=2)
    # 去掉最后一列（可能全为空？）但实际我们写入时都有数据，保留全部
    # 列数：前8列 + 3*len(sorted_regions) + 1（总计）
    num_cols = 8 + 3 * len(sorted_regions) + 1
    df_data = df_data.iloc[:, :num_cols]

    # 构建列名
    std_col = '标准化'  # 实际列名可能带比例，但我们可以用内部名称，因为 HTML 生成会重新生成带比例的列名
    cfg_col = '配置化'
    cus_col = '定制化'
...
# ========== 修改主函数 ==========
def main() -> None:
    parser = argparse.ArgumentParser(description='分组统计工具')
    parser.add_argument('--from-excel', action='store_true',
                        help='从已有的分组统计结果.xlsx 生成 HTML（不重新统计）')
    args = parser.parse_args()

    if args.from_excel:
        # ---------- 从 Excel 生成 HTML ----------
        excel_file = '分组统计结果.xlsx'
        if not os.path.exists(excel_file):
            print(f"错误：未找到 {excel_file}，请先运行默认模式生成。")
            sys.exit(1)
        try:
            # 读取国内数据
            df_dom = rebuild_df_from_excel(excel_file, '国内')
            dom_html = df_to_html_with_merges(df_dom, "国内", font_size=FONT_SIZE_HTML)
            # 读取国际数据
            df_int = rebuild_df_from_excel(excel_file, '国际')
            ...
            print(f"HTML 已生成并打开：{html_file}")
        except Exception as e:
            print(f"从 Excel 生成 HTML 时出错：{e}")
            sys.exit(1)
        return

    # ---------- 默认模式：统计并生成 Excel 和 HTML ----------
    data_path = find_data_file()
    if data_path is None:
        print("错误：在 data 文件夹中未找到包含“接单明细”的 Excel 文件。")
        sys.exit(1)
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        # ===== 新增：写入元数据 Sheet =====
        meta_data = {
            'sorted_regions_dom': df_dom.attrs['sorted_regions'],
            'region_data_dom': df_dom.attrs['region_data'],
            'sorted_regions_int': df_int.attrs['sorted_regions'],
            'region_data_int': df_int.attrs['region_data'],
        }
        # 由于两个 sheet 的 sorted_regions 可能不同，我们分别存储，但为了简洁，可以存储两个，但 rebuild 时只需对应 sheet 的
        # 我们可以存储全局的，但按 sheet 区分更好。这里我们存储为两个 JSON 字符串，分别对应国内和国际。
        meta_df = pd.DataFrame({
            'key': ['sorted_regions_dom', 'region_data_dom', 'sorted_regions_int', 'region_data_int'],
            'value': [
                json.dumps(df_dom.attrs['sorted_regions']),
                json.dumps(df_dom.attrs['region_data']),
                json.dumps(df_int.attrs['sorted_regions']),
                json.dumps(df_int.attrs['region_data']),
            ]
        })
